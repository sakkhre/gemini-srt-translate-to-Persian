...
این اپلیکیشن یک ابزار پیشرفته و هوشمند برای ترجمه فایل‌های زیرنویس (SRT) به زبان **فارسی محاوره‌ای و سینمایی** است. با بهره‌گیری از مدل هوش مصنوعی **Gemini 2.5/3**، این برنامه تفاوت‌های فرهنگی و اصطلاحات عامیانه را درک کرده و زیرنویسی کاملاً طبیعی تولید می‌کند.
...
### ۳. تنظیم کلید API
برنامه برای کارکرد نیاز به API Key گوگل دارد. در محیط ویندوز، می‌توانید آن را به عنوان یک Environment Variable تعریف کنید یا در فایل تنظیمات پروژه قرار دهید.
...
npm start
```
برنامه به طور خودکار در آدرس `http://localhost:3000` باز خواهد شد.
...
*   **مشکل فونت:** اگر فونت‌های فارسی به درستی نمایش داده نمی‌شوند، اطمینان حاصل کنید که فونت Vazirmatn در سیستم لود شده است (این برنامه به صورت خودکار از Google Fonts استفاده می‌کند).
*   **خطای API:** در صورت بروز خطای ۴۰۱ یا ۴۰۳، اعتبار API Key خود را در کنسول گوگل بررسی کنید.
*   **Encoding:** برای جلوگیری از به هم ریختگی حروف فارسی، فایل‌های خروجی همیشه با انکودینگ **UTF-8** ذخیره می‌شوند.

---

## 🤝 مشارکت در پروژه
اگر پیشنهادی برای بهبود مدل ترجمه یا رابط کاربری دارید، خوشحال می‌شویم یک **Pull Request** ارسال کنید یا در بخش **Issues** آن را مطرح کنید.
